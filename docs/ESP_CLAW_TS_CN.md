...
> **ESP-IDF 版本基线：** 推荐并已验证的版本是 **ESP-IDF 5.5.4**。N16R8
> 固件已在此版本完成构建、刷机与实机验证。ESP-IDF 6.x 不属于当前已验证基线，
> 可能需要额外的兼容性调整。
...
进入 **基本设置 > Wi-Fi 设置**，可按优先级保存最多 5 个 2.4 GHz Wi-Fi。
首次升级时，旧版保存的单个 SSID/密码会自动迁移为第 1 个网络，无需重新输入。

- 断线或启动时，设备扫描一次，先按列表从上到下尝试当前可见的已保存网络，再直接尝试
  未出现在扫描结果里的已保存网络，避免扫描结果被截断或不完整时漏掉实际可连接的网络。
- 如果全部尝试都失败，配网热点会继续保留；设备每 30 秒重新扫描并按优先级重试完整列表，
  直到某个网络成功取得 IP。
- 当前连接健康时不会为了更高优先级网络自动漫游，避免打断网页、对话和 Tailscale。
- “立即连接”会主动切换网络，当前网页可能随之断开；指定网络失败后会尝试其余已保存网络。
- 已保存密码只写入、不回显。密码框留空会保留同 SSID 的原密码；勾选“开放网络”才会明确清除密码。

保存 Wi-Fi 列表后无需重启。AP 名称、AP 密码等基础网络设置仍按页面提示重启生效。
STA 成功联网后，配网热点通常会自动关闭。

如果扫描 SSID 时提示 `esp_wifi_scan_start failed`，可以直接手工填写 SSID；
这个错误本身不代表 STA 一定无法连接。

如果配网热点消失，且串口反复显示 `Brownout detector was triggered`，请先断开 GPIO
外设，并更换确认可靠的数据线和供电充足的 USB 接口。Wi-Fi 射频初始化时触发 Brownout
属于供电完整性问题，不能据此判断 Wi-Fi 列表或 FATFS 已损坏。
...
恢复出厂设置会删除应用、全部 Wi-Fi 节点和 ESP-Claw TS 的已保存配置。
