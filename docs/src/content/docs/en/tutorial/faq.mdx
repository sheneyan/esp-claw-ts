...
**What to try:**
1. Open the Web config page under SoftAP, check the saved profiles and their
   priority, and connect to 2.4 GHz Wi-Fi. [➡️Web config](@lang/tutorial/web-config)
2. Move the board closer to the AP; check antenna connection if present.
3. If the serial log repeatedly reports `Brownout detector was triggered`,
   disconnect GPIO peripherals and test with a known-good data cable and a
   sufficiently powered USB port. This is a power fault, not a Wi-Fi-profile or
   FATFS error.

When every saved profile fails, the provisioning SoftAP should remain
available and the device retries the list after 30 seconds. **Connect now** can
temporarily make the current page unreachable because the device deliberately
drops its existing station connection before trying the selected profile.
