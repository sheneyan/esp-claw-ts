...
Its mutation tools require an explicit request from the current user and `user_confirmed: true`. Hostname, auth key, login server, enablement, and device identity stay on the settings page; activating the Skill does not grant those mutations.
