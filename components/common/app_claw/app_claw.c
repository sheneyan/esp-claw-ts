...
static esp_err_t init_skills(const app_claw_storage_paths_t *paths)
{
    ...
    err = claw_skill_add_directory(paths->skills_root_dir);
    if (err != ESP_OK) {
        ESP_LOGW(TAG,
                 "User skills directory is unavailable or damaged (%s); continuing with system skills",
                 esp_err_to_name(err));
    }
    return ESP_OK;
}
...
static esp_err_t build_storage_paths(app_claw_storage_paths_t *paths)
{
    ...
    ESP_RETURN_ON_ERROR(claw_paths_join(CLAW_PATH_SYSTEM, ".recovery/router_rules/router_rules.json",
                                        paths->router_rules_fallback_path,
                                        sizeof(paths->router_rules_fallback_path)),
                        TAG, "fallback router rules path too long");
...
esp_err_t app_claw_start(const app_claw_config_t *config)
{
    ...
    router_config.fallback_rules_path = paths.router_rules_fallback_path;
#endif
